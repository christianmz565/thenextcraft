...
if (!getEnvVar(envDevContent, "CONVEX_SELF_HOSTED_ADMIN_KEY") && fs.existsSync(envDevPath)) {
  envDevContent = upsertEnvVar(envDevContent, "CONVEX_SELF_HOSTED_ADMIN_KEY", adminKey);
  fs.writeFileSync(envDevPath, envDevContent);
  console.log("Saved CONVEX_SELF_HOSTED_ADMIN_KEY to .env.dev");
}
...
let webEnvContent = fs.existsSync(webEnvLocalPath)
  ? fs.readFileSync(webEnvLocalPath, "utf8")
  : "# Generated by `bun run convex:admin-key`. Do not commit.\n";
