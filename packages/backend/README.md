...
- **Convex Auth - manual setup required:** The Convex CLI does **not** support self-hosted deployments for Convex Auth yet. Follow the manual instructions at https://labs.convex.dev/auth/setup/manual — set `AUTH_GOOGLE_ID`, `AUTH_GOOGLE_SECRET`, `SITE_URL` directly in the backend container env / `docker-compose.yml` (or your infra's env config) rather than via `convex env set`. CLI commands `npx convex env set` / `convex env list` will not work against a self-hosted deployment.
