...
- **Convex Auth env setup:** Set `AUTH_GOOGLE_ID`, `AUTH_GOOGLE_SECRET`, and `SITE_URL` in the self-hosted Convex backend via `convex env set <KEY> <VALUE> --url http://127.0.0.1:3210 --admin-key <admin-key>`. Setting process/container environment variables in `docker-compose.yml` does not inject them into the V8 function runtime where Convex Auth executes.
